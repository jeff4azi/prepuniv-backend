...
app.post("/api/wallet/topup/initiate", authenticateRequest, async (req, res) => {
  try {
...
app.post("/api/webhooks/flutterwave", async (req, res) => {
  try {
    ...
    const expectedHash = crypto
      .createHmac("sha256", FLUTTERWAVE_WEBHOOK_SECRET || FLUTTERWAVE_SECRET_KEY)
    ...
    try {
      const verifyResp = await flutterwaveAxios.get(`/transactions/${transaction_id}/verify`);
  ...
  try {
    ...
    const { data: attempt, error: attemptErr } = await supabase
      ...
      .insert({
        ...
        quiz_id: quiz.id,
        is_timed: !!is_timed,
        time_allowed_seconds: is_timed ? (time_allowed_seconds || quiz.time_allowed_seconds) : null,
      })
      .select()
      .single();
...
app.post("/api/creator/payout-request", authenticateRequest, async (req, res) => {
  try {
    const { amount } = req.body;
    ...
    if (!profile.is_approved_creator) {
      return res.status(403).json({ error: "Only approved creators can request payouts" });
    }
    ...
    if (earningsBalance < amount) {
      return res.status(400).json({ error: "Amount exceeds available earnings balance" });
    }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (updateErr) {
        return res.status(500).json({ error: "Failed to update payout request" });
      }
      ...
      const account_number = payoutRequest.bank_account_number || creator.bank_account_number;
      ...
      try {
        ...
        try {
          const verifyResp = await flutterwaveAxios.get(`/transfers/${transferRef}`);
          verifiedStatus = verifyResp.data?.data?.status || transferStatus;
          transferMessage = verifyResp.data?.message || transferResp.data?.message || "";
        } catch (vErr) {
          transferMessage = vErr.message;
        }

        if (verifiedStatus === "SUCCESSFUL" || verifiedStatus === "successful" || verifiedStatus === "NEW") {
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (error) {
        return res.status(500).json({ error: "Failed to reject payout request" });
      }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (error) {
        return res.status(500).json({ error: "Failed to reject creator application" });
      }
